...
def run_stage(run_dir: Path, config: dict[str, Any], experiment_manifest: Path, command: list[str]) -> None:
    ...
    validate_jsonl(items_path, ROOT / "modules/stage_6_items/schemas/item_spec_v0_1.schema.json", label="simulation input ItemSpec")
    ...
    validate_jsonl(observed_path, interaction_schema, label="simulation output Interaction")
    write_stage_manifest(
        ...
        started_utc=started,
        command=command,
        inputs=[items_path, q_path],
        configs=[experiment_manifest, parameter_path, interaction_schema],
        code=[Path(__file__)],
