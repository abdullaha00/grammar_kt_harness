...
from shared.utils.io import ROOT, read_jsonl, repo_path, require_new_directory, sha256_file, utc_now, write_jsonl
...
def run_generation(items_dir: Path, run_dir: Path, config: dict[str, Any], experiment_manifest: Path, command: list[str]) -> None:
    ...
    write_stage_manifest(
        ...
        started_utc=started,
        command=command,
        inputs=[projection_path, inventory_path, realizations_path, cells_path, mappings_path],
        configs=[experiment_manifest, template_path, lexicon_path, rules_path],
        code=[Path(__file__), ROOT / "modules" / "items" / "helpers.py", ROOT / "modules" / "realization" / "engine.py"],
        outputs=[candidates_path, units_path],
        details={
