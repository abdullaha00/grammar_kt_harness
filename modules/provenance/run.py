...
from shared.utils.io import ROOT, read_jsonl, require_new_directory, utc_now, write_json, write_jsonl
...
def build_graph(
    ...
    for egp_id, row in sorted(mapping_by_source.items()):
        ...
        edges.append(provenance_edge("SOURCE_TO_PHASE1", f"EGP:{egp_id}", phase1, {"sha256": row["phase1_sha256"]}))
...
def run_stage(run_dir: Path, config: dict[str, Any], experiment_manifest: Path, command: list[str]) -> None:
    ...
    write_stage_manifest(
        ...
        started_utc=started,
        command=command,
        inputs=[source_path, mappings_path, mapping_path, source_edges_path, realizations_path, items_path, q_edges_path, interactions_path],
        configs=[experiment_manifest, ROOT / "shared" / "schemas" / "provenance_edge.schema.json"],
        code=[Path(__file__)],
        outputs=[edges_path, audit_path],
