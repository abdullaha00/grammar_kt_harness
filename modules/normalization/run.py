...
def _prompt(phase: int, config: dict[str, Any], replacements: dict[str, str]) -> str:
    ...
    if unresolved:
        raise ValueError(f"unresolved normalization prompt placeholders: {unresolved}")
    return (
        "FROZEN GRAMMAR SCHEMA\n=====================\n"
        f"{schema}\nFROZEN NORMALIZATION RULEBOOK\n=============================\n"
        f"{rulebook}\nFROZEN PHASE {phase} PROMPT\n=====================\n{rendered}"
    )
...
def _annotate(
    *,
    ...
    raw_root = output / "raw" / f"phase{phase}"
    parsed_path = output / "parsed" / f"phase{phase}" / f"{unit_id}.json"
    ...
    for attempt in range(1, int(config["max_attempts"]) + 1):
        ...
        metadata["transition_validation_errors"] = transition_errors
        ...
        if mapping is not None and not errors:
            write_json(parsed_path, mapping)
            return {
                "unit_id": unit_id,
                "egp_id": egp_id,
