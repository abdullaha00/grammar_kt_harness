...
from ..generation.items import item_bank_fingerprint
...
def difficulty(measurement_opportunity_id: str, low: float, high: float) -> float:
    """Assign latent difficulty at the opportunity level, never the surface level."""

    fraction = int(hashlib.sha256(measurement_opportunity_id.encode()).hexdigest()[:16], 16) / float(
        0xFFFFFFFFFFFFFFFF
    )
...
def opportunity_outcome_fingerprint(rows: list[dict[str, Any]]) -> str:
    """Fingerprint the surface-invariant scientific outcome projection."""
    ...
    return event_stream_fingerprint(projected)
...
def project_oracle_items(
    ...
    opportunity_by_id = {
        row["measurement_opportunity_id"]: row for row in opportunities
    }
    projections = []
    for item in sorted(items, key=lambda row: row["item_id"]):
        opportunity = opportunity_by_id.get(item["measurement_opportunity_id"])
        if opportunity is None:
            raise RuntimeError(f"oracle item projection has unknown opportunity: {item['item_id']}")
        if opportunity["canonical_cell_id"] != item["canonical_cell_id"]:
            raise RuntimeError(f"oracle item projection cell mismatch: {item['item_id']}")
        evidence_record = structural_evidence(
        ...
        for definition in definitions:
            ...
            activated, rule_evidence = evaluate_structural_rule(
                definition["activation_rule"], evidence_record
            )
            evidence[feature_id] = {
                "activated": activated,
        ...
        for _ in range(int(params["learners_per_profile"])):
            ...
            for _pass in range(int(params["item_passes_per_learner"])):
                item_ids = sorted(
                    item_by_id,
                    key=lambda item_id: (
                        item_by_id[item_id]["measurement_opportunity_id"],
                        item_id,
                    ),
                )
            ...
            measurement_exposures: Counter[str] = Counter()
            if retain:
                found = True
                learners.append(
                    {
            ...
            for sequence, item_id_value in enumerate(order, 1):
                ...
                measurement_id = item["measurement_opportunity_id"]
                active = oracle_by_item[item_id_value]
                pre = {feature_id: mastery[feature_id] for feature_id in active}
                item_difficulty = difficulty(
                    measurement_id, params["difficulty_min"], params["difficulty_max"]
                )
                complexity_penalty = float(params["oracle_complexity_penalty"]) * max(
                    0, len(active) - 1
                )
                ...
                probability = params["probability_floor"] + params["probability_span"] * sigmoid(z)
                measurement_exposures[measurement_id] += 1
                draw = deterministic_draw(
                    int(params["seed"]), learner_id, measurement_id,
                    measurement_exposures[measurement_id], "ordinary"
                )
                    ...
                    observed.append(
                        {
                            ...
                            "measurement_opportunity_id": measurement_id,
                            "canonical_cell_id": item["canonical_cell_id"],
                            "canonical_split": item["canonical_split"],
...
def simulate_compositional_records(
    ...
    def ordered_ids(split: str) -> list[str]:
        return sorted(
            (item_id for item_id, item in item_by_id.items() if item["canonical_split"] == split),
            key=lambda item_id: (
                item_by_id[item_id]["measurement_opportunity_id"],
                item_id,
            ),
    ...
    development_ids = ordered_ids("development")
    compositional_ids = ordered_ids("compositional_holdout")
    novel_ids = ordered_ids("novel_feature_holdout")
    ...
    for profile_name, profile in params["profiles"].items():
        for _ in range(int(params["learners_per_profile"])):
            ...
            for sequence, item_id_value in enumerate(acquisition_order, 1):
                ...
                measurement_exposures[measurement_id] += 1
                draw = deterministic_draw(
                    protocol_seed, learner_id, measurement_id,
                    measurement_exposures[measurement_id], "acquisition"
                )
                correct = int(draw < probability)
                temporal_split = (
                    "train"
            ...
            for probe_type, probe_ids, destination in (
                ...
                for repetition in range(1, probe_repetitions + 1):
                    for item_id_value in probe_ids:
                        ...
                        event_id = f"COMP_PROBE_{probe_type.upper()}_{learner_id}_{probe_sequence:04d}"
                        draw = deterministic_draw(
                            protocol_seed, learner_id, measurement_id, repetition, probe_type
                        )
                        correct = int(draw < probability)
                        timestamp = (
...
def run(run_dir: Path, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    opportunities = read_jsonl(
        run_dir / "measurement" / "measurement_opportunities.jsonl"
    )
    ...
    projections, feature_ids = project_oracle_items(items, opportunities, params)
    ...
    audit.update(
        {
            ...
            "measurement_opportunity_bank_sha256": opportunity_bank_fingerprint(
                opportunities
            ),
            "fold_id": manifest["fold_id"],
            "base_event_stream_sha256": event_stream_fingerprint(observed),
            "opportunity_outcome_sha256": opportunity_outcome_fingerprint(observed),
    ...
    write_json(
        compositional_output / "audit.json",
        {
            **compositional_audit,
            ...
            "measurement_opportunity_bank_sha256": audit[
                "measurement_opportunity_bank_sha256"
            ],
            "fold_id": manifest["fold_id"],
            "oracle_representation_id": params["oracle_representation_id"],
    ...
    return {
        ...
        "opportunity_outcome_sha256": audit["opportunity_outcome_sha256"],
