...
def project_interactions(
    ...
    for base in sorted(
        ...
        projection = projection_by_item[base["item_id"]]
        if projection["measurement_opportunity_id"] != base["measurement_opportunity_id"]:
            raise RuntimeError("candidate projection changes MeasurementOpportunity identity")
        active = list(projection["kc_ids"])
...
def project_compositional_interactions(
    ...
    for base in sorted(
        ...
        if projection_by_item[base["item_id"]]["measurement_opportunity_id"] != base["measurement_opportunity_id"]:
            raise RuntimeError("acquisition projection changes MeasurementOpportunity identity")
    ...
    for base in probe_events:
        compositional_base_event(base, label=base["event_id"])
        if projection_by_item[base["item_id"]]["measurement_opportunity_id"] != base["measurement_opportunity_id"]:
            raise RuntimeError("probe projection changes MeasurementOpportunity identity")
        active = list(projection_by_item[base["item_id"]]["kc_ids"])
        supported = [kc_id for kc_id in active if kc_id in development_supported]
        cold = [kc_id for kc_id in active if kc_id not in development_supported]
...
def run(run_dir: Path, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    item_projections = read_jsonl(run_dir / "knowledge" / "item_kc_projection.jsonl")
