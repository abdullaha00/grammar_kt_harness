...
def select(settings: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    source = repo_path(settings["path"])
    if sha256_file(source) != settings["sha256"]:
        raise RuntimeError("external EGP source SHA-256 differs from experiments/base.yaml")
    ids = [line for line in repo_path(settings["sample_ids"]).read_text(encoding="utf-8").splitlines() if line]
    records = read_jsonl(source)
    if len(records) != int(settings["records"]):
        raise RuntimeError(f"external source has {len(records)} records; expected {settings['records']}")
    ...
    if len(selected) != int(settings["selected_descriptors"]):
        raise RuntimeError(f"selected {len(selected)} descriptors; expected {settings['selected_descriptors']}")
    metadata = read_jsonl(settings["sample_metadata"])
    units = read_jsonl(settings["annotation_units"])
...
def run(run_dir: Path, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    selected, metadata, units = select(settings)
