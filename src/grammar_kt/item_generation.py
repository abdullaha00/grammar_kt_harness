...
from .io import read_jsonl, resource, write_json, write_jsonl
...
def _sampling_spec(
    ...
    replicate: int,
    offset: int,
    imperative_realisations: dict[str, list[dict[str, Any]]],
) -> dict[str, Any]:
    cell = opportunity["cell"]
    if cell["clause"] == "imperative":
        choices = imperative_realisations[opportunity["canonical_cell_id"]]
...
def generate_candidates(
    *,
    ...
    imperative_realisations: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in realisations:
        if row["cell"]["clause"] == "imperative":
            imperative_realisations[row["spec"]["canonical_cell_id"]].append(row)
    for rows in imperative_realisations.values():
    ...
    for kc_offset, kc_id in enumerate(sorted(by_kc)):
        ...
        for replicate in range(replicates):
            ...
            for adjustment in range(15):
                spec = _sampling_spec(kc_id, opportunity, replicate, kc_offset + adjustment, imperative_realisations)
...
def run_generation(items_dir: Path, run_dir: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    realisations_path = run_dir / "realisation" / "realisations.jsonl"
    template_path = resource("items", "families", config["family"], ".txt")
    lexicon_path = resource("realisation", "lexicons", config["realisation_lexicon"], ".jsonl")
    ...
    realisations = read_jsonl(realisations_path)
    ...
    candidates = generate_candidates(
        ...
        replicates=int(config["replicates_per_kc"]),
        development_replicates=int(config["development_replicates"]),
    )
    ...
    realisations_by_cell: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for row in realisations:
        realisations_by_cell[row["spec"]["canonical_cell_id"]].append(row)
    for item in ordered:
        ...
        write_json(
            unit_dir / "input.json",
            {
                ...
                "realisations_for_cell": realisations_by_cell[item["canonical_cell_id"]],
