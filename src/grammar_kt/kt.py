...
def history_features(
    ...
    if event_items - set(by_item):
        raise ValueError(
            f"KC projection lacks event items: {sorted(event_items - set(by_item))}"
        )
    ...
    for event in sorted(events, key=lambda row: (row["learner_id"], row["sequence_index"])):
        ...
        overall = (learner_correct[learner] + alpha) / (
            learner_attempts[learner] + alpha + beta
        )
        rates = [
            (kc_correct[(learner, kc_id)] + alpha)
            / (kc_attempts[(learner, kc_id)] + alpha + beta)
            for kc_id in active
        ]
...
def _bkt(
    ...
    for event in sorted(events, key=lambda row: (row["learner_id"], row["sequence_index"])):
        ...
        if active:
            mean_mastery = (
                sum(state[learner][kc_id] for kc_id in active) / len(active)
            )
